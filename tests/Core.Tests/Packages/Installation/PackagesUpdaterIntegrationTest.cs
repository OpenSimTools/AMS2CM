...
public class PackagesUpdaterIntegrationTest : AbstractFilesystemTest
{
    ...
    public void Apply_UpdatesMods()
    {
        ...
        recordedState.Should().BeEquivalentTo(new Dictionary<string, PackageInstallationState>
        {
            ["A"] = new(Time: fakeUtcInstallationDate, FsHash: 2, Partial: false, Files: [
                "AF",
                "AF2"
            ])
        });
    ...
    public void Apply_RestoresFilesPreviouslyShadowedByUninstalledMod()
    {
        Apply(
            new Dictionary<string, PackageInstallationState>
            {
                ...
                ["B"] = new(Time: null, FsHash: 2, Partial: false, Files: [
                    "SF",
                    "BF1",
                ])
            },
        ...
        recordedState.Should().BeEquivalentTo(new Dictionary<string, PackageInstallationState>
        {
            ["A"] = new(Time: fakeUtcInstallationDate, FsHash: 1, Partial: false, Files: [
                "SF",
                "AF1"
            ])
        });
